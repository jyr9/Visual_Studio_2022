...
void addEdge(Adjacency_list_graph& graph, int tail, int head);
...
void addEdge(Adjacency_list_graph& graph, int tail, int head) {
    ...
    newNode->Adjacency_vertex = head;
    newNode->next = nullptr;

    Edge_node** current = &graph.adjacency_list[tail].First_node;
    while (*current != nullptr && (*current)->Adjacency_vertex < head) {
...
void Initialize_adjacency_list_graph(Adjacency_list_graph& adjacency_list_graph) {
    ...
    {
        cout << "请输入" << edge_number << "条无向边" << endl;
        cout << "第" << i + 1 << "条：";
        int tail, head; //输入无向边
        cin >> tail >> head;

        // 添加tail -> head 的边，同时保持顺序
        addEdge(adjacency_list_graph, tail, head);
        // 添加head -> tail 的边，同时保持顺序
        addEdge(adjacency_list_graph, head, tail);
    }
}
